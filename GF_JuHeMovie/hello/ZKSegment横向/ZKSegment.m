...
#define ZK_ItemFontSize 19.0f
#define ZK_ItemMargin 20.0f
#define ZK_ItemPadding self.frame.size.height
...
+ (ZKSegment *)zk_segmentWithFrame:(CGRect)frame style:(ZKSegmentStyle)style {
    ...
    ZKSegment *zkSegment = [[ZKSegment alloc] zk_initWithFrame:frame style:style];
    
    zkSegment.scrollEnabled = NO;
    
    zkSegment.pagingEnabled = YES;
...
- (void)buttonStyleFromSegmentStyle {
    switch (self.segmentStyle) {
        case ZKSegmentLineStyle:
            ...
            break;
        case ZKSegmentRectangleStyle:
            self.buttonStyleY = (self.frame.size.height - ZK_ItemPadding)/2;
            self.buttonStyleHeight = ZK_ItemPadding;
            self.buttonStyleCornerRadius = 8.0f;
...
- (void)itemClick:(id)sender {
    ...
    if (self.buttonSelected != button) {
        ...
        if (self.block) {
            ...
            _block(selectedItem,selectedIndex);
        }
...
-(void)zk_itemClickBlock:(zk_itemClickBlock)block{
